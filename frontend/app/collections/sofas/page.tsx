...
export default function SofasCollectionPage() {
  ...
  useEffect(() => {
    const fetchProducts = async () => {
      try {
        setLoading(true)
        const response = await fetch(`${API_BASE_URL}/api/Product?includeDrafts=false&categoryName=SOFAS`)
        if (response.ok) {
                  ...
                  <Link href={`/product/${product.id}`}>
                    ...
                    <div className={`p-4 ${viewMode === "list" ? "flex-1" : ""}`}>
                      ...
                      {product.variants && product.variants.length > 0 && (() => {
                        // Parse variant attributes to extract available colors and sizes
                        ...
                        // Extract unique colors and sizes from available variants
                              ...
                              <div>
                                ...
                                <div className="flex flex-wrap gap-2">
                                  {Array.from(availableColors).map((color, idx) => {
                                    const colorVariant = product.variants.find((v: ProductVariant) => {
                                      try {
                                        if (v.attributes) {
                                          ...
                                          return (attrs[colorAttrName] || v.color) === color && v.isAvailable !== false
                                        }
                                        return v.color === color && v.isAvailable !== false
                                      } catch {
                                        return v.color === color && v.isAvailable !== false
                                      }
                                    })
                                    ...
                                    const isSelected = selectedVariants[product.id]?.id === colorVariant?.id
                                    ...
                                    return (
                                      <button
                                        ...
                                        className={`flex items-center gap-1.5 px-2 py-1 rounded-full border transition-all ${
                                          isSelected 
                                            ? "border-[#ed6b3e] bg-orange-50" 
                                            : "border-gray-200 bg-white hover:border-gray-300"
                                        }`}
                                        title={`${color} - Click to view images`}
                                      >
                                        <div
                                        ...
                                        {colorVariant && (() => {
                                          try {
                                            ...
                                            if (colorVariant.mediaUrls) {
                                              const parsed = typeof colorVariant.mediaUrls === 'string' 
                                                ? JSON.parse(colorVariant.mediaUrls) 
                                                : colorVariant.mediaUrls
                                              variantImages = Array.isArray(parsed) ? parsed : []
                                            }
                                            if (variantImages.length === 0 && colorVariant.imageUrl) {
                                              variantImages = [colorVariant.imageUrl]
                                            }
